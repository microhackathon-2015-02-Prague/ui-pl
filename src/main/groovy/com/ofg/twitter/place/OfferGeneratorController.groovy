...
import com.ofg.twitter.place.extractor.OfferGeneratorWorker
...
@Slf4j
@RestController
@RequestMapping('/offergenerator')
@TypeChecked
@Api(value = "pairId", description = "Collects places from tweets and propagates them to Collerators")
class OfferGeneratorController {

    private final OfferGeneratorWorker offerGeneratorWorker

    @Autowired
    OfferGeneratorController(OfferGeneratorWorker offerGeneratorWorker) {
        this.offerGeneratorWorker = offerGeneratorWorker
    }
    ...
    Callable<Void> getPlacesFromTweets(@PathVariable @NotNull long pairId, @RequestBody @NotNull List<Tweet> tweets) {
        return {
            offerGeneratorWorker.collectAndPropagate(pairId, tweets)
        }
    }
