...
import com.ofg.twitter.place.extractor.LoanAppServiceWorker
...
@Slf4j
@RestController
@RequestMapping('/loanappservice')
@TypeChecked
@Api(value = "pairId", description = "Collects places from tweets and propagates them to Collerators")
class LoanAppServiceController {

    private final LoanAppServiceWorker loanAppServiceWorker

    @Autowired
    LoanAppServiceController(LoanAppServiceWorker loanAppServiceWorker) {
        this.loanAppServiceWorker = loanAppServiceWorker
    }
    ...
    Callable<Void> getPlacesFromTweets(@PathVariable @NotNull long pairId, @RequestBody @NotNull List<Tweet> tweets) {
        return {
            loanAppServiceWorker.collectAndPropagate(pairId, tweets)
        }
    }
