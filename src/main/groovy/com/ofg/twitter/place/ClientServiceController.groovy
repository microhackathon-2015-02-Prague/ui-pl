...
import com.ofg.twitter.place.extractor.ClientServiceWorker
...
class ClientServiceController {
    ...
    private final ClientServiceWorker clientServiceWorker

    @Autowired
    ClientServiceController(ClientServiceWorker clientServiceWorker) {
    ...
    Callable<Void> getPlacesFromTweets(@PathVariable @NotNull long pairId, @RequestBody @NotNull List<Tweet> tweets) {
        return {
            clientServiceWorker.collectAndPropagate(pairId, tweets)
        }
    }
